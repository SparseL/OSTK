function [RMSEtrain,RMSEtest,t,C_ini,Sigma_ini,W]=MBGD_RDA(XTrain,yTrain,XTest,yTest,alpha,rr,P,nMFs,nIt,Nbs)
...
for it=1:nIt
    ...
    if (~mod(it,500))
    % Training RMSE
     RMSEtrain(kkk)=sqrt(sum((yTrain(idsTrain(idsGoodTrain))-yPred(idsGoodTrain)).^2)/sum(idsGoodTrain));
    % Test RMSE
